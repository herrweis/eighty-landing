...
// Mobile timing - black stays until text is fully settled, then reveals
const MOBILE_SPAN = 1.8;
const MOBILE_DRAW_END = 0.25;
const MOBILE_FILL_END = 0.4;
const MOBILE_VIDEO_END = 0.85; // Black holds until text settles (was 0.9)
const MOBILE_REVEAL_START = 0.88; // Start revealing after text settled (was 0.78)
const MOBILE_REVEAL_END = 0.98; // Complete reveal at very end
...
function setupFigureDraw() {
  ...
  const render = () => {
    ...
    // Detect mobile breakpoint (matches SCSS @include mobile)
    const isMobile = window.innerWidth < 900;

    // Use mobile or desktop timing constants
    const span = isMobile ? MOBILE_SPAN : SPAN;
    const drawEnd = isMobile ? MOBILE_DRAW_END : DRAW_END;
    const fillEnd = isMobile ? MOBILE_FILL_END : FILL_END;
    const videoEnd = isMobile ? MOBILE_VIDEO_END : VIDEO_END;
    const revealStart = isMobile ? MOBILE_REVEAL_START : REVEAL_START;
    const revealEnd = isMobile ? MOBILE_REVEAL_END : REVEAL_END;
    ...
    // Draw: stagger each stroke across [0, drawEnd].
    ...
    // Fill: strokes fade out / masked water fades in across [drawEnd, fillEnd].
    const fillIn = clamp((p - drawEnd) / (fillEnd - drawEnd), 0, 1);
    // Video drains to carbon around the filled shapes across [fillEnd, videoEnd].
    const videoOut = clamp((p - fillEnd) / (videoEnd - fillEnd), 0, 1);
    // Reveal: the full picture fades in across [revealStart, revealEnd] — after
    // a hold on [videoEnd, revealStart] so the carbon composition reads first.
    const revealIn = clamp((p - revealStart) / (revealEnd - revealStart), 0, 1);
    ...
    const fillOut = clamp((p - revealEnd) / (1 - revealEnd), 0, 1);
